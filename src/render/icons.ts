...
// ready for the gemini connector (asl-alg); wire it into PLATFORM_ICONS
// and PLATFORM_LABELS when the Platform union gains "gemini".
...
// dependence); the ring is two opposite-winding circles, the dot a third.
...
// Record<Platform, …>: compile-time exhaustive, so widening the union
// (e.g. gemini) breaks the build until an icon decision is made here.
...
// Human label for the icon's aria-label/title; unknown ids pass through raw
// (they're arbitrary strings — callers rely on platformIcon's escaping).
export function platformLabel(platform: string): string {
  return (PLATFORM_LABELS as Record<string, string>)[platform] ?? platform;
}

// Complete inline <svg> for a platform's mark: 14px, currentColor, labeled
// for AT (role/aria-label) and hover (title). String-typed on purpose —
// render call sites may carry future config-driven platform ids, which
// degrade to the neutral fallback instead of crashing. Alignment comes from
// the stylesheet's .platform-icon class (html.ts), not inline style.
export function platformIcon(platform: string): string {
  const d = (PLATFORM_ICONS as Record<string, string>)[platform] ?? FALLBACK_PATH;
  const label = esc(platformLabel(platform));
  return `<svg class="platform-icon" width="14" height="14" viewBox="0 0 24 24" fill="currentColor" role="img" aria-label="${label}"><title>${label}</title><path d="${d}"/></svg>`;
}
