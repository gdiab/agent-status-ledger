...
6. ~~Should the report distinguish “agent did work” from “agent helped human think through work”?~~ **Resolved 2026-07-16** (shipped, asl-cey). Yes. When the Engram connector is enabled, each profile's observed dialogue is classified from message/edit ratios and tool-call density — any session with observed code edits (or tool-dense activity) reads as build work; dialogue-dominant profiles are labeled thinking help on the agent card (`interactionKind`), so a pure conversation is never reported like a build run. The same pass quotes the final question of a run that ended awaiting the user (`awaitingQuestion`, through the `sanitizeTapeText` choke point of §13), so a needs-human card shows the decision being waited on, not just a flag. Quoted dialogue appears on the full report surfaces (Markdown/HTML/JSON), and — per the §13 carve-out decided 2026-07-17 (asl-94g) — the email digest's exceptions triage carries the sanitized, truncated awaiting-question line per exception agent; the digest otherwise carries no transcript text.
